...
class NotificationService:
    ...
    def notify_emergency(self, *, patient: Patient, call: Call) -> None:
        """SMS the doctor first, then place a CALL-E warning call."""
        ...
        if not to_number:
            logger.warning(
                "No doctor contact for patient %s; skipped emergency SMS and warning call",
                patient.id,
            )
            self._mark_skipped(sms, "No doctor contact on file")
            self._mark_skipped(warning, "No doctor contact on file")
            return

        self._send_sms(sms, to_number=to_number, body=body, dry_run=call.dry_run)
